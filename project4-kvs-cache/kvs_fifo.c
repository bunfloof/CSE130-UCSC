...
kvs_fifo_t* kvs_fifo_new(kvs_base_t* kvs, int capacity) {
  ...
  kvs_fifo->keys = calloc(capacity, sizeof(char*)); // allocate memory for keys arr
  kvs_fifo->values = calloc(capacity, sizeof(char*)); // alocate mem for vals arr
  kvs_fifo->dirty = calloc(capacity, sizeof(bool)); // newly intizlized dirty arr
...
void kvs_fifo_free(kvs_fifo_t** ptr) { // memory unsafe function to free memory from FIFO cache
...
int kvs_fifo_set(kvs_fifo_t* kvs_fifo, const char* key, const char* value) { // fn to add key-value pair to FIFO cache
  if (kvs_fifo->capacity == 0) return kvs_base_set(kvs_fifo->kvs_base, key, value); // case for capacity 0 set val in disk and return result
  // proceed below to look in cache ------
  
  for (int i = 0; i < kvs_fifo->size; ++i) { // iterate through cache
    int index = (kvs_fifo->head + i) % kvs_fifo->capacity; // calculate index based on head and current loop counter wrapped around cache capacity
    if (strcmp(kvs_fifo->keys[index], key) == 0) { // if key found in cache, then update its value
      free(kvs_fifo->values[index]); // free old val associated with key
      kvs_fifo->values[index] = strdup(value); // update val associated with key in cache
      kvs_fifo->dirty[index] = true; // set dirty flag because value updated in cache but not persisted to disk
      return 0; // return success without updating the disk store
    }

  } // proceed below if the key was not found in the cache ------

  if (kvs_fifo->size == kvs_fifo->capacity) { // if the cache is full, evict the head
    if (kvs_fifo->dirty[kvs_fifo->head]) { // if the entry being evicted is dirty, persist it to disk
      ...
      if (rc != 0) return rc;
      //kvs_fifo->dirty[kvs_fifo->head] = false; // reset dirty flag
    ...
    kvs_fifo->head = (kvs_fifo->head + 1) % kvs_fifo->capacity; // update head index to point to next entry in circular arr cache
  ...
  kvs_fifo->keys[kvs_fifo->tail] = strdup(key); // add new key-value pair to tail of cache
  kvs_fifo->values[kvs_fifo->tail] = strdup(value); 
  kvs_fifo->dirty[kvs_fifo->tail] = true; // mark new key-value pair as dirt because it hasn't persisted to disk yet
...
int kvs_fifo_get(kvs_fifo_t* kvs_fifo, const char* key, char* value) { // fn to get value associated with a key from FIFO cache
  if (kvs_fifo->capacity == 0) return kvs_base_get(kvs_fifo->kvs_base, key, value); // case for capacity 0 set val in disk and return result
  // proceed below to look in cache ------
...
int kvs_fifo_flush(kvs_fifo_t* kvs_fifo) { // fn to flush FIFO cache by persisting all dirty entries to disk and evicting all entries
  ...
  while (kvs_fifo->size > 0) { // while entries in circular cache exist
    if (kvs_fifo->dirty[kvs_fifo->head]) { // if entry at head is dirty
      rc = kvs_base_set(kvs_fifo->kvs_base, kvs_fifo->keys[kvs_fifo->head], kvs_fifo->values[kvs_fifo->head]); // persist to disk and save return code
      if (rc != 0) return rc; // if kvs_base_set return code indicate failure, then return error code
    }

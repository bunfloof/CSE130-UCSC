 ...
 * - Using memmove instead of regular shift to safely handle overlap when
 *shifting array.
 *
 * Usage:
...
kvs_lru_t* kvs_lru_new(kvs_base_t* kvs, int capacity) {
  kvs_lru_t* kvs_lru =
      malloc(sizeof(kvs_lru_t));  // allocate memory base of key-value store
...
void kvs_lru_free(
    kvs_lru_t** ptr) {  // memory unsafe function to free memory from LRU cache
...
int kvs_lru_set(kvs_lru_t* kvs_lru, const char* key,
  ...
  for (int i = 0; i < kvs_lru->size;
    ...
    if (strcmp(kvs_lru->keys[i], key) ==
        0) {                     // if key is found in cache, update value
      ...
      // regular method used instead of memmove because we have small controlled
      // shift with no risk of source and destination memory areas overlapping
      for (int j = i; j < kvs_lru->size - 1;
           ++j) {  // move all keys, values, dirty flags one position towards
                   // head of cache
        kvs_lru->keys[j] = kvs_lru->keys[j + 1];
        kvs_lru->values[j] = kvs_lru->values[j + 1];
        kvs_lru->dirty[j] = kvs_lru->dirty[j + 1];
      }
      kvs_lru->keys[kvs_lru->size - 1] =
          temp_key;  // move updated key, values, dirty flags to end of cache
  ...
  if (kvs_lru->size == kvs_lru->capacity) {  // if the key is not found in the
                                             // cache and cache is full
    if (kvs_lru->dirty[0]) {  // if the least recently used (LRU) entry is
                              // dirty, update in disk
      int rc =
          kvs_base_set(kvs_lru->kvs_base, kvs_lru->keys[0], kvs_lru->values[0]);
      if (rc != 0)
        return rc;  // if updating disk store fails indicated by a return code
                    // other than success, return return-code
      kvs_lru->replacement_count++;  // increment replacement_count here because
                                     // a dirty pair was replaced
    }

    free(kvs_lru->keys[0]);    // free least recently used (LRU) key
    free(kvs_lru->values[0]);  // free least recently used (LRU) val

    // using the memmove function to move all keys, values, and dirty flags one
    // position towards head of the cache memmove used here instead of manually
    // shifting each entry because memmove safely handles overlap between source
    ...
    memmove(kvs_lru->dirty, kvs_lru->dirty + 1,
            (kvs_lru->size - 1) * sizeof(bool));
  } else {
    ++kvs_lru->size;  // else if cache not full, increase its size
  }

  kvs_lru->keys[kvs_lru->size - 1] =
      strdup(key);  // add new key to end of cache (most recently used position)
  kvs_lru->values[kvs_lru->size - 1] = strdup(
      value);  // add new val to end of cache (most recently used position)
...
int kvs_lru_get(
  ...
  for (int i = 0; i < kvs_lru->size;
    ...
    if (strcmp(kvs_lru->keys[i], key) == 0) {  // if key is present in cache
      strcpy(value,
             kvs_lru->values[i]);  // copy value to provided pointer buffer
      ...
      // shift with no risk of source and destination memory areas overlapping
      for (int j = i; j < kvs_lru->size - 1;
           ++j) {  // move all keys, values, and dirty flag one position towards
                   // head of cache
        kvs_lru->keys[j] = kvs_lru->keys[j + 1];
        kvs_lru->values[j] = kvs_lru->values[j + 1];
        kvs_lru->dirty[j] = kvs_lru->dirty[j + 1];
      ...
      kvs_lru->keys[kvs_lru->size - 1] =
          temp_key;  // move accessed key-valued pair to end of cache (most
  ...
  if (rc == 0) {  // if key not in disk, add key-value pair to cache
    if (kvs_lru->size == kvs_lru->capacity) {  // if cache is full
      if (kvs_lru->dirty[0]) {                 // if key dirty
        rc = kvs_base_set(kvs_lru->kvs_base, kvs_lru->keys[0],
                          kvs_lru->values[0]);  // probe return code
        if (rc != 0)
          return rc;  // if return code is not success, then return return-code
        kvs_lru->replacement_count++;  // increment replacement_count here
                                       // because a dirty pair was replaced
      }

      free(kvs_lru->keys[0]);    // free least recently used (LRU) entry
      free(kvs_lru->values[0]);  // free least recently used (LRU) entry

      // using the memmove function to move all keys, values, and dirty flags
      // one position towards head of the cache memmove used here instead of
      // manually shifting each entry because memmove safely handles overlap
      // between source and destination memory areas
      memmove(kvs_lru->keys, kvs_lru->keys + 1,
              (kvs_lru->size - 1) * sizeof(char*));
      memmove(kvs_lru->values, kvs_lru->values + 1,
              (kvs_lru->size - 1) * sizeof(char*));
    ...
    kvs_lru->keys[kvs_lru->size - 1] = strdup(
        key);  // add new key at end of cache (most recently used position)
    kvs_lru->values[kvs_lru->size - 1] = strdup(
        value);  // add new val at end of cache (most recently used position)
    kvs_lru->dirty[kvs_lru->size - 1] =
        false;  // mark new pair as clean/false entry becuase it was just from
                // the disk
  }
...
int kvs_lru_flush(kvs_lru_t* kvs_lru) {
  for (int i = 0; i < kvs_lru->size;
       ++i) {                 // iterate through all elements in cache
    if (kvs_lru->dirty[i]) {  // if entry is dirty
      int rc = kvs_base_set(kvs_lru->kvs_base, kvs_lru->keys[i],
                            kvs_lru->values[i]);  // probe for return code

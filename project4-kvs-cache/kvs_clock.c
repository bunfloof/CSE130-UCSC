...
kvs_clock_t* kvs_clock_new(kvs_base_t* kvs, int capacity) {
  kvs_clock_t* kvs_clock =
      malloc(sizeof(kvs_clock_t));  // base of key-value store
  ...
  kvs_clock->ref_bits =
      calloc(capacity, sizeof(int));  // newly initialized reference bits array
  kvs_clock->dirty =
      calloc(capacity, sizeof(bool));  // newly initialized dirt array
...
void kvs_clock_free(kvs_clock_t** ptr) {  // memory unsafe function to free
                                          // memory from COCK cache
...
int kvs_clock_set(kvs_clock_t* kvs_clock, const char* key, const char* value) {
  for (int i = 0; i < kvs_clock->size;
    ...
    if (strcmp(kvs_clock->keys[i], key) ==
        0) {  // if key in cache, update value
      // printf("[i] Setting key '%s' which already exists in the cache at index
      // %d\n", key, i);
  ...
  if (kvs_clock->size <
      kvs_clock->capacity) {  // if cache not full, add key-value pair to cache
    ...
    while (kvs_clock->ref_bits[kvs_clock->cursor]) {  // find key-value pair
      ...
      kvs_clock->cursor =
          (kvs_clock->cursor + 1) % kvs_clock->capacity;  // move cursor
    }

    if (kvs_clock->dirty[kvs_clock->cursor]) {  // if key-value pair is dirty,
                                                // write back to disk
...
int kvs_clock_get(kvs_clock_t* kvs_clock, const char* key, char* value) {
  ...
  if (rc == 0) {  // if key not in base disk, add key-value pair to cache;
                  // should we count empty files? yes-rohan
    if (kvs_clock->size ==
        kvs_clock->capacity) {  // if cache full, replace a key-value pair using
                                // cock algorithm
      while (kvs_clock->ref_bits[kvs_clock->cursor]) {  // find key-value pair
                                                        // with ref bit of 0
        kvs_clock->ref_bits[kvs_clock->cursor] = 0;     // reset ref bit
      ...
      kvs_clock->dirty[kvs_clock->cursor] =
          false;  // set to false because value is identical to the one on disk
      kvs_clock->ref_bits[kvs_clock->cursor] =
          1;  // set to 1 whenever a GET operation results in a cache miss
      ...
      kvs_clock->dirty[kvs_clock->size] =
          false;  // reset to false because the value is identical to the one on
                  // disk
      kvs_clock->ref_bits[kvs_clock->size] =
          1;  // set to 1 whenever a GET operation results in a cache miss
...
int kvs_clock_flush(
    kvs_clock_t* kvs_clock) {  // fn to flush COCK cache by persisting all dirty
                               // entries to disk and evicting all entries
  int result = 0;              // initialize return code

  for (int i = 0; i < kvs_clock->size;
       ++i) {                   // iterate through all entries in cache
    if (kvs_clock->dirty[i]) {  // if entry is dirty
      result |= kvs_base_set(
          kvs_clock->kvs_base, kvs_clock->keys[i],
          kvs_clock->values[i]);    // was previously "result |=" bitwise OR
                                    // operator to accumlate error return-codes

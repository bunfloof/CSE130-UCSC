...
int kvs_clock_flush(kvs_clock_t* kvs_clock) {
  ...
  for (int i = 0; i < kvs_clock->size; ++i) {
    result |= kvs_base_set(kvs_clock->kvs_base, kvs_clock->keys[i], kvs_clock->values[i]); // this line potential stats issue FLUSH operation should persist (ie gwdwtf., updates or stores) the modified entries in memory to the disk

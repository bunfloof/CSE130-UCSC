 ...
 * - For checking, the sed command replaces "bobcat" with "cat" in the output
 *for the sake of checking with diff
 *
 * Usage:
 * ./bobcat [FILE]...
 *
 ...
 *
 * Checks:
 * diff <(./bobcat your_arguments_here 2>&1 | sed 's/bobcat/cat/g') <(cat
 *your_arguments_here 2>&1)
 *
 *********************************************************************************/
#include <err.h>
...
#define BUFFER_SIZE \
  1024  // theoretically speaking, the memory usage of the bobcat program should
        // always stay below 1 MB, as it uses a fixed-size buffer of 1024 bytes
        // (1 KB) for reading and writing data.

// helper function to process the file with the provided file descriptor (fd)
// and filename
void process_file(int fd, const char *filename, bool *error_occurred) {
  char buffer[BUFFER_SIZE];
  ssize_t rcount;

  while ((rcount = read(fd, buffer, BUFFER_SIZE)) >
         0) {  // read the file in chunks of BUFFER_SIZE
    ssize_t wcount = 0;
    while (wcount < rcount) {  // write the read bytes to the standard output in
                               // a loop to handle partial writes
      ...
      if (result < 0) {  // handle write errors
        ...
        *error_occurred = true;
        if (errno == EPIPE)
          exit(EXIT_FAILURE);  // exit if the error is a broken pipe
        return;
      }
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {  // no arguments provided, process stdin
    ...
    for (int i = 1; i < argc; i++) {
      if (!double_dash &&
          strcmp(argv[i], "--") ==
              0) {  // encountered a double dash, mark it and continue
        double_dash = true;
        continue;
      }
      ...
      if (!double_dash &&
          strcmp(argv[i], "-") ==
              0) {  // single dash without a double dash, process stdin
        process_file(STDIN_FILENO, "-", &error_occurred);
        ...
        if (fd < 0) {  // if the file cannot be opened
          if (strcmp(argv[i], "-") != 0 ||
              !double_dash) {  // if it's not a single dash after a double dash,
                               // print a warning
            warn("%s", argv[i]);
            error_occurred = true;
    ...
    if (!processed_file && double_dash && !non_existent_file) {
      process_file(
          STDIN_FILENO, "-",
          &error_occurred);  // if no files have been processed and a double
                             // dash was encountered, process stdin
